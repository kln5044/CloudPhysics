...
def x_from_Tp(T,p):
	""" Calculates x from equation 1, using
	T in Kelvin and p in mb. """
	x = T - (skew_slope*np.log(p))
	return x

def y_from_p(p):
	""" Calculates y from equation 2, using
	p in mb. """
	y = -(np.log(p))
	return y

def p_from_y (y):
	""" Calculates p from equation 1, using
	p in mb. """
	p = (np.exp(-y))
	return p 

def T_from_xp (x,p):
	""" Calculates temperature in Kelvin
	from x and p, which is in mb. """
